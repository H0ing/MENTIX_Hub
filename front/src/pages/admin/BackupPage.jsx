    ...
    <div>
      ...
      {tab === 'instant' && (
        <div className="grid grid-cols-[1.3fr_1fr] gap-5">
          ...
          <div className="bg-white border border-[#ECE9F4] rounded-[14px] p-[22px]">
            <h3 className="text-[15.5px] font-bold m-0 mb-4 flex items-center">Estimated File Size <MvpBadge /></h3>
            <div className="border-[1.5px] border-dashed border-[#ECE9F4] rounded-[14px] p-[18px] flex items-center justify-between bg-[#FBFAFD]">
              <div><div className="text-[13.5px] font-bold">Preview Unavailable</div><div className="text-[12px] text-[#8B8B9E] mt-1">Size estimation ships post-MVP.</div></div>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'schedule' && (
        <div className="bg-white border border-[#ECE9F4] rounded-[14px] p-[22px] max-w-[520px]">
      ...
      <Modal open={!!restoreModal} title="Restore from this backup?" onClose={() => setRestoreModal(null)}
        footer={<><Button onClick={() => setRestoreModal(null)}>Cancel</Button><Button variant="primary" onClick={async () => { await backupService.restoreBackup(restoreModal); setRestoreModal(null); showToast('Restore completed'); }}>Restore</Button></>}>
        <p className="m-0">Current data will be overwritten.</p>
      </Modal>
    </div>
  );
}
