...
export async function create({ username, email, password_hash, full_name, year, major, role }) {
  const sql = `
    INSERT INTO users (username, email, password_hash, full_name, year, major, role)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `;
  return user(sql, [username, email, password_hash, full_name, year || null, major || null, role || 'student']);
}
